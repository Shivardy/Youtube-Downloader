...
cursor:pointer;
	...
	<script>
		function myfun(a){
		 window.location.href = "youtube.php?v=" +a;
		}
		function myfun1(a){
	
	 window.location.href = "youtube.php?playlist=" +a;
		}
		</script>
<body>
<nav class="navbar navbar-default">
  <div class="container-fluid">
    <div class="navbar-header">
      <a class="navbar-brand" href="youtube">
...
<div align="center" class="embed-responsive embed-responsive-16by9">
 <iframe width="560" height="315" src="https://www.youtube.com/embed?listType=playlist&list='.$playlist.'" frameborder="0" allowfullscreen></iframe>
</div>

</div>';
echo '<div id="rbox" class="col-sm-4" style="overflow-y:scroll;overflow-x:hidden; min-width:390px;
    height:430px; border:1px solid grey;border-radius:3px;"><h2 align="center"style="border-bottom:1px solid grey; ">Download Playlist</h2>';
$playres = file_get_contents("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=".$playlist."&key=APIKEY&maxResults=20");
$playres = json_decode($playres,true);
foreach ($playres['items'] as $plr) {
$videoid= $plr['snippet']['resourceId']['videoId'];
$data = file_get_contents("https://www.youtube.com/get_video_info?video_id=$videoid");
	parse_str($data);

	if(stripos($data,"url_encoded_fmt_stream_map")){
		$arr=explode(",",$url_encoded_fmt_stream_map);
$stats=file_get_contents("https://www.googleapis.com/youtube/v3/videos?id=".$videoid."&part=statistics&key=APIKEY");

	$searchResponse = json_decode($stats,true);
	$title = substr($title,0,46).'...';

	echo '<button style="word-wrap:break-word !important; max-width:360px;" class="btn btn-info btn-block" type="button" data-toggle="collapse" data-target="#'.$videoid.'" >
 <h5 class="faqp"> '.$title.'<h5>
</button><br>';
echo '<div class="collapse" id="'.$videoid.'">
  <div class="well"><p>Views= '.$view_count.' | Likes= '.$searchResponse['items']['0']['statistics']['likeCount'].' | Dislikes= '.$searchResponse['items']['0']['statistics']['dislikeCount'].'</p>
...
echo '<div class="row"><div class="col-xs-11" style="border:1px solid grey;padding-top:5px;padding-right:10px;border-radius:3px;background-color:white;">';
echo '<div style="margin:0 auto;">';
$query=clean($playres['items']['0']['snippet']['title']);
$response = file_get_contents("https://www.googleapis.com/youtube/v3/search?part=snippet&q={$query}&type=video&key=APIKEY&maxResults=12");

$searchResponse = json_decode($response,true);
foreach ($searchResponse['items'] as $searchResult) {?>
<div id='related' style='float:left;' onclick="myfun('<?php echo $searchResult['id']['videoId']; ?>');">
<?php
echo'<img class="img-responsive" style="padding:10px;max-width:225px;" 
...
$data = file_get_contents("https://www.youtube.com/get_video_info?video_id=$vid");
	...
	if(stripos($data,"url_encoded_fmt_stream_map")){
		$arr=explode(",",$url_encoded_fmt_stream_map);
$stats=file_get_contents("https://www.googleapis.com/youtube/v3/videos?id=".$vid."&part=statistics&key=APIKEY");
...
<?php
echo '<div class="row"><div class="col-xs-8" style="border:1px solid grey;padding-top:5px;padding-right:10px;border-radius:3px;background-color:white;">';
echo '<div style="margin:0 auto;">';
$query=clean($title);
$response = file_get_contents("https://www.googleapis.com/youtube/v3/search?part=snippet&q={$query}&type=video&key=APIKEY&maxResults=12");
...
echo '<div class="col-xs-4" style="border:1px solid grey;padding-top:5px;border-radius:3px;padding-left:5px;background-color:white;">';

echo '<h2 align="center"style="border-bottom:1px solid grey; ">Related Playlists</h2>';

$playres = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=snippet&id=".$vid."&key=APIKEY");
$playres = json_decode($playres,true);
$chanelid=$playres['items']['0']['snippet']['channelId'];

$playres = file_get_contents("https://www.googleapis.com/youtube/v3/playlists?part=snippet&channelId=".$chanelid."&key=APIKEY&maxResults=8");
$playres = json_decode($playres,true);

foreach ($playres['items'] as $plres) {

	
?>
	<div id="relateddiv" onclick="myfun1('<?php echo $plres['id']; ?>');"><?php
$playrs = file_get_contents("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=".$plres['id']."&key=APIKEY");
$playrs = json_decode($playrs,true);
echo '<img class="img-responsive" style="float:left;clear:both;padding:10px;max-width:115px;" 
src="'.$plres['snippet']['thumbnails']['medium']['url'].'" alt="Youtube Video">.<b style="margin-top:60px;">'.$plres['snippet']['title'].'</b><p style="padding-bottom:20px;">'.$playrs['pageInfo']['totalResults']
.' Videos</p>';
...
echo "<style>body{background-color:#efefef;}.container .col-sm-10{background-color:white;}</style>";
	...
	$response = file_get_contents("https://www.googleapis.com/youtube/v3/search?part=snippet&q={$query}&type=video&key=APIKEY&maxResults=30");
...
?>
  ...
  <div class="jumbotron">
    ...
    <p align="center" class="lead">Get Youtube videos to your Devices !</p> <br>
		<form  role="search" method="post" action="youtube.php">
  <div class="form-group">
